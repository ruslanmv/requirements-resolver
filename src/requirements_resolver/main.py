...
def run_cli_mode(backend, args):
    ...
    logger_thread = threading.Thread(target=console_logger, args=(log_queue,), daemon=True)
    logger_thread.start()
...
def run_gui_mode(backend):
    ...
    if not is_tkinter_installed():
        print("Error: Tkinter is not installed, which is required for the GUI.", file=sys.stderr)
        print("Please install it for your system (e.g., 'sudo apt-get install python3-tk' on Debian/Ubuntu).", file=sys.stderr)
        sys.exit(1)
